...
std::vector<size_t> SearchServer::findDocsWithAllWords(const std::vector<std::string>& words) {
    ...
    for (size_t i = 1; i < words.size() && !doc_ids.empty(); i++) {
        ...
        std::sort(current_doc_ids.begin(), current_doc_ids.end());
    ...
    const std::vector<std::string>& words)
{
    ...
    for (const auto& word : words) {
        word_entries[word] = index.GetWordCount(word);
    }
    ...
    for (size_t i = 0; i < doc_ids.size(); i++) {
        ...
        for (const auto& [word, entries] : word_entries) {
            for (const auto& entry : entries) {
                if (entry.doc_id == doc_id) {
                    abs_relevance[i] += static_cast<float>(entry.count);
                    break;
                }
            }
        }
    ...
    for (size_t i = 0; i < doc_ids.size(); i++) {
        if (max_relevance > 0) {
            float relative_rank = abs_relevance[i] / max_relevance;
            result.push_back({ doc_ids[i], relative_rank });
        }
    }
