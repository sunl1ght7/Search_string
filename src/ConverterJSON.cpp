...
std::vector<std::string> ConverterJSON::GetTextDocuments() {
    ...
    for (const auto& filePath : config["files"]) {
        ...
        try {
            documents.push_back(readFile(pathStr));
        }
        catch (const std::exception& e) {
            std::cerr << "Error reading file " << pathStr << ": " << e.what() << std::endl;
        }
    }
...
std::string ConverterJSON::readFile(const std::string& path) {
    ...
    if (!std::filesystem::exists(fullPath)) {
        std::filesystem::path resourcePath = std::filesystem::path("resources") / path;
        fullPath = resourcePath.string();
